...
const search = async (
  ...
  if (name) {//if name is not undefined, conduct search
    let ext = 'ext';
    if (req.query.ext === undefined) {//if no ext was defined
    ...
    if (result) {// if image exists
      ...
      try{
        ...
        if (convertExists){//if the file exists in the folder exists:
         console.log(`A Converted ${name}.jpg file already exists`);
         res.send(//serve the existing file
      ...
      catch(error){//if thumbs folder does not exist:
        await fs.mkdir('src/imgs/thumbs')//create the folder
        const thumbs = await fs.readdir('src/imgs/thumbs')
